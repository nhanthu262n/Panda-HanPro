(() => {
  ...
  const S = {
    ...
    recordBlob:null, recordUrl:"", autoTimer:0, currentWord:null, recognitionError:"", recognitionEnded:false, reports:[], speakingItems:[], variant:0
  };
...
`;
  ...
  async function openVocabulary(m){createShell("vocab-intro",m);S.items=wordList(S.mission).slice(0,18).map(itemFromWord).filter(x=>x.char);S.index=0;S.answers=[];S.phase="learn";if(!S.items.length){document.getElementById("ptCoachSkillContent").innerHTML=`<div class="ptcs-card">${T("Không có từ vựng Excel được giao cho ngày này.","No Excel vocabulary is assigned to this Day.")}</div>`;return}renderVocabLearn()}
