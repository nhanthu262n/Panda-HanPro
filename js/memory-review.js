...
`;document.head.appendChild(el)}
  ...
  async function renderTeacherHistory(host,students){
    const db=window.PandaHanFirebase?.firestore;if(!host||!db)return;style();const box=document.createElement('section');box.className='pm-card';box.innerHTML='<h3>Kết quả ghi nhớ của học sinh</h3>';host.appendChild(box);
    for(const s of students){const card=document.createElement('details');card.className='pm-card';card.innerHTML=`<summary>${esc(s.name||s.uid)}</summary><button class="pm-btn" type="button">Tải lịch sử và kết quả</button><div></div>`;box.appendChild(card);card.querySelector('button').onclick=async()=>{const area=card.querySelector('div');area.textContent='Đang tải…';try{let last=null,data=[];do{let query=db.collection('learningAttempts').doc(s.uid).collection('attempts').orderBy('createdAt','desc').limit(500);if(last)query=query.startAfter(last);const snap=await query.get();data.push(...snap.docs.map(d=>({...d.data(),attemptId:d.id,synced:true})));last=snap.docs.length===500?snap.docs.at(-1):null}while(last);area.innerHTML=resultsHtml(data)}catch(e){area.textContent='Chưa tải được: '+e.message}}}
  }
  ...
  window.PanTutorMemory={snapshot,replay,assess,getState,openPractice,renderPractice,mountDashboard,historyHtml,resultsHtml,feedback,renderTeacherHistory};
