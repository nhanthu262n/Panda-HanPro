...
(function () {
  ...
  function renderFlowPanel() {
    ...
    panel.querySelectorAll("[data-srs-grade]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (typeof window.gradeWord === "function") window.gradeWord(word.char, grade);
      window.dispatchEvent(new CustomEvent("pandahan-srs-updated", { detail: { source: "daily-vocabulary-flow", char: word.char, grade, dayNumber: flow.dayNumber } }));
      flow.index += 1;
  ...
  function startVocabularyFlow(words, dayNumber = null) {
    ...
    flow = { words: normalized, index: 0, dayNumber: Number(dayNumber || 0) || null };
