...
function text(n){if(!n||n.nodeType!==3||isAiTutorNode(n)||isPinyinNode(n)||isPhoneticsNode(n))return;const raw=n.nodeValue||"",s=raw.trim();if(s==="English learning guidance"){n.nodeValue="";return;}if(!s||!hasVi(s))return;let out=convert(s);if(hasVi(out)){
...
function attrs(el){if(isAiTutorNode(el)||isPinyinNode(el)||isPhoneticsNode(el))return;["placeholder","title","aria-label"].forEach(a=>{const v=el.getAttribute?.(a);if(v&&hasVi(v)){let x=convert(v);el.setAttribute(a,hasVi(x)?"Learning content":x)}})}
