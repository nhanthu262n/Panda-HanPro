...
function attrs(el){if(isAiTutorNode(el)||isPinyinNode(el)||isPhoneticsNode(el)||isQuestNode(el))return;["placeholder","title","aria-label"].forEach(a=>{const v=el.getAttribute?.(a);if(v&&hasVi(v)){let x=convert(v);el.setAttribute(a,hasVi(x)?v:x)}})}
