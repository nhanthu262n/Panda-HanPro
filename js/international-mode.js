...
function isPracticeQuizNode(n){
  const el=n?.parentElement || (n?.nodeType===1?n:null);
  if(!el)return false;
  return !!el.closest?.('#quizView,#qCard,#qContent,.quiz-card,.quiz-question,.quiz-options,.quiz-explain');
}
...
function text(n){if(!n||n.nodeType!==3||isAiTutorNode(n)||isPinyinNode(n)||isPhoneticsNode(n)||isQuestNode(n)||isPracticeQuizNode(n))return;const raw=n.nodeValue||"",s=raw.trim();if(s==="English learning guidance"){n.nodeValue="";return;}if(!s||!hasVi(s))return;let out=convert(s);if(hasVi(out)){
  ...
  if(/[一-鿿]/.test(out)){const parts=out.split(/([·|/]|\s+—\s+)/);const kept=parts.filter(p=>!hasVi(p));out=kept.join("").replace(/^[\s·|/—]+|[\s·|/—]+$/g,"").trim()||"Chinese learning item"}
    ...
    // Never erase interactive controls. Direct-English source should cover known UI,
    // but preserving the original label is safer than rendering an empty button.
    if(/^(BUTTON|OPTION|LABEL|SUMMARY)$/.test(tag)) out=s;
    else out="";
  }
}
