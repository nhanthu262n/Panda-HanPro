...
function translateKnownLegacyUi(root = document.body) {
  if (!root) return;
  root.querySelectorAll("button, label, option, summary, h1, h2, h3, h4, p, small, span, div").forEach((el) => {
    if (el.children.length) return;
    const original = el.dataset.i18nLegacyVi || String(el.textContent || "").trim();
    if (!LEGACY_UI_EN[original]) return;
    if (!el.dataset.i18nLegacyVi) el.dataset.i18nLegacyVi = original;
    el.textContent = LANG_MODE === "en" ? LEGACY_UI_EN[original] : original;
  });
}
...
function resolveVocabularyMistake(char) {
  ...
  const now = Date.now();
  const queue = readMistakeQueueRaw();
  const item = queue.filter((entry) => entry.char === keyChar && (mistakeIsOpen(entry) || mistakeIsDue(entry, now))).sort((a, b) => Number(b.lastWrongAt || 0) - Number(a.lastWrongAt || 0))[0];
  if (!item) return;
...
window.PandaHanMistakes = { getQueue: getVocabularyMistakeQueue, getOpenQueue: getOpenVocabularyMistakeQueue, getAllQueue: getAllVocabularyMistakes, record: recordVocabularyMistake, resolve: resolveVocabularyMistake, retryDays: MISTAKE_RETRY_DAYS.slice() };
