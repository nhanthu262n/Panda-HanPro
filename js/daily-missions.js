(() => {
  ...
  const TASK_META = {
    ...
    phonetics_core: {
      titleVi: "Pinyin Bootcamp · Phonetics Core", titleEn: "Pinyin Bootcamp · Phonetics Core", icon: "🎵", minutes: 10,
      instructionVi: "Complete the scored standalone phonetics task for this exact Day. A score of 30% or higher saves a green check; it never replaces the Pinyin Tone Quest unlock gate.",
      instructionEn: "Complete the scored standalone phonetics task for this exact Day. A score of 30% or higher saves a green check; it never replaces the Pinyin Tone Quest unlock gate."
    },
    listening: {
      titleVi: "Listening Lab", titleEn: "Listening Lab", icon: "🎧", minutes: 8,
      instructionVi: "Open the standalone AI Coach Listening Lab for this exact Excel Day. Model audio auto-plays, the learner answers audio-first questions, and the real score is saved as Day evidence.",
      instructionEn: "Open the standalone AI Coach Listening Lab for this exact Excel Day. Model audio auto-plays, the learner answers audio-first questions, and the real score is saved as Day evidence."
    },
    speaking: {
      titleVi: "Speaking / Read-aloud Lab", titleEn: "Speaking / Read-aloud Lab", icon: "🗣️", minutes: 10,
      instructionVi: "Open the standalone speaking cards for this exact Excel Day: listen to the model, read, record, replay and receive a pronunciation-rubric score.",
      instructionEn: "Open the standalone speaking cards for this exact Excel Day: listen to the model, read, record, replay and receive a pronunciation-rubric score."
    },
    reading_writing: {
  ...
  function buildTasks(day, adaptivePlan = null, vocabPhase = {}) {
    ...
    // Excel is the master curriculum. Day 1-10 is Pinyin Bootcamp: give the
    // learner one independent, scored phonetics task inside AI Coach before the
    // Excel listening/speaking work. It is evidence only, never an unlock gate.
    if (dayNumber <= 10) add("phonetics_core");
  ...
  function startTask(type) {
    ...
    if (type === "phonetics_core") {
      if (window.PandaHanCoachSkills?.openPhoneticsCore) window.PandaHanCoachSkills.openPhoneticsCore(m);
      else openExcelPhoneticsSession(m, "phonetics_core");
    }
    else if (type === "vocab-intro") {
      if (window.PandaHanCoachSkills?.openVocabulary) window.PandaHanCoachSkills.openVocabulary(m);
      else { window.switchTab?.("practice"); setTimeout(() => window.startAdaptiveVocabularyLesson?.(m.adaptivePlan?.introWords?.length ? m.adaptivePlan.introWords : (m.chainVocabulary || []), m.dayNumber), 80); }
    }
    else if (type === "wrong-review" || type === "mistake_review") {
      if (window.PandaHanCoachSkills?.openMistakeReview) window.PandaHanCoachSkills.openMistakeReview(m);
      else window.startMistakeReview?.();
    }
    else if (type === "quiz") {
      ...
      setTimeout(async () => {
        ...
        window.setTimeout(() => window.PandaHanQuestParts?.openQuestDay?.(exactDay).catch?.(() => {}), 450);
        const input = document.getElementById("questDaySearch");
        if (input) input.value = String(exactDay);
      }, 120);
    }
    else if (type === "listening") {
      if (window.PandaHanCoachSkills?.openListening) window.PandaHanCoachSkills.openListening(m);
      else { console.warn("Standalone Listening Lab is not ready; using the legacy fallback."); if (Number(m.dayNumber) <= 10) openExcelPhoneticsSession(m, "listening"); else { window.switchTab?.("practice"); setTimeout(() => window.startQuizForWords?.(m.chainVocabulary || [], scheduledContext), 80); } }
    }
    else if (type === "speaking") {
      if (window.PandaHanCoachSkills?.openSpeaking) window.PandaHanCoachSkills.openSpeaking(m);
      else { console.warn("Standalone Speaking Lab is not ready; using the legacy fallback."); if (Number(m.dayNumber) <= 10) openExcelPhoneticsSession(m, "speaking"); else { try { localStorage.setItem("pandahan_phonetics_focus", type); } catch (_) {} window.switchTab?.("pinyin"); } }
    }
    else if (type === "reading_writing") {
      if (window.PandaHanCoachSkills?.openReadingWriting) window.PandaHanCoachSkills.openReadingWriting(m);
      else if (Number(m.dayNumber) <= 10) openExcelPhoneticsSession(m, "reading_writing");
      else { window.switchTab?.("practice"); setTimeout(() => window.startWriteGame?.(scheduledContext), 80); }
    }
    else if (type === "srs") {
      if (window.PandaHanCoachSkills?.openSrs) window.PandaHanCoachSkills.openSrs(m);
      else { window.setPracticeMode?.("scheduled"); window.switchTab?.("reviewIntro"); }
    }
  ...
  function workbookTaskDescription(taskId, curriculum, langEn) {
    ...
    if (langEn && (taskId === "listening" || taskId === "speaking")) {
      const translated = window.PandaHanCoachSkills?.taskEnglish?.(value, taskId);
      return `Excel task: ${translated || (taskId === "listening" ? "Complete the standalone listening assignment for this Day." : "Complete the standalone speaking assignment for this Day.")}`;
    }
  ...
  function renderRequiredChecklist(m, langEn) {
    ...
    return `<div style="margin-top:10px;padding:9px 10px;border:1px solid #e9d5ff;border-radius:11px;background:#fff;"><b>${langEn ? "Learning evidence" : "Evidence học tập"}</b><div style="font-size:11px;color:#64748b;margin-top:3px;">${langEn ? "Every non-Quest row opens a scored standalone AI Coach task. Passing saves a green check, but NEVER blocks or unlocks the next day." : "Mọi mục ngoài Quest đều mở task AI Coach độc lập có chấm điểm. Đạt sẽ tích xanh, nhưng KHÔNG BAO GIỜ chặn hoặc mở ngày kế tiếp."}</div>${rows}</div>`;
  }
  ...
  function renderAttemptHistory(dayNumber,langEn){
    const rows=window.PanTutorAttemptHistory?.dayRows?.(dayNumber)||[];
    if(!rows.length)return "";
    const titles={phonetics_core:"Ngữ âm",listening:"Nghe",speaking:"Nói",srs:"Ôn tập SRS","vocab-intro":"Từ vựng",reading_writing:"Đọc/Viết",mistake_review:"Ôn lỗi sai",remediation:"Luyện theo chẩn đoán",teacherDraft:"Câu chờ đánh giá"};
    return `<details style="margin-top:10px;padding:10px;border:1px solid #dbeafe;border-radius:11px;background:white"><summary><b>${langEn?"Attempt history":"Lịch sử làm bài"} · ${rows.length}</b></summary>${rows.slice(0,20).map(row=>`<div style="padding:6px;border-bottom:1px solid #eee;font-size:12px"><b>${esc(titles[row.taskId]||row.taskId)}</b> · ${esc(new Date(row.createdAt).toLocaleString("vi-VN"))} · ${row.taskId==="teacherDraft"?"Chờ chấm":row.scorePercent+"/100"} · ${row.taskId==="teacherDraft"?"Chờ giáo viên":row.passed?(langEn?"Pass":"Đạt"):(langEn?"Retry":"Chưa đạt")} · ${row.synced?(langEn?"Synced":"Đã đồng bộ"):(langEn?"Pending sync":"Chờ đồng bộ")}<details><summary>${langEn?"Answers":"Xem từng câu"}</summary>${(row.items||[]).map((item,i)=>`<div>${i+1}. ${esc(item.input||"—")} → ${esc(item.expected||"—")} · ${item.correct===true?(langEn?"Correct":"Đúng"):item.correct===false?(langEn?"Incorrect":"Sai"):`${esc(item.score??"—")}/100`}</div>`).join("")}</details></div>`).join("")}</details>`;
  }
  ...
  function renderCoach(container, compact = false) {
    ...
    container.querySelector("[data-open-memory-review]")?.addEventListener("click",()=>window.PanTutorMemory?.openPractice());
  }
  ...
  ["pandahan-vocab-phase-updated", "pandahan-schedule-updated", "pandahan-mistake-recorded", "pandahan-mistake-resolved", "pandahan-learning-evaluation", "pandahan-ai-coach-assessment", "pandahan-language-changed", "pantutor-attempt-saved"].forEach((eventName) => window.addEventListener(eventName, refreshCoachPlanSoon));
